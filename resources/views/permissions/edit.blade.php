                            ...
                            <div class="card-body">
                                ...
                                <div class="col">
                                    <div class="input-group mb-3">
                                        <input type="text" class="form-control rounded-3" id="permissionName"
                                            placeholder="Permission Name" name="name" value="{{ $permission->name }}">
                                    </div>
                                    <div class="col">
                                        <select class="form-select rounded-3" id="assignedTo" name="user_id">
                                            ...
                                            @foreach ($users as $user)
                                                <option value="{{ $user->id }}"
                                                    @if ($user->id == $permission->user_id) selected @endif>{{ $user->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="d-flex gap-1 align-items-center">
                                        <button class="btn btn-outline-primary rounded-3 btn-sm" type="submit">Update
                                            Permission</button>
                                        <a href="{{ route('permissions.index') }}" class="btn btn-secondary btn-sm">Back To
                                            Permissions</a>
                                    </div>
                                </div>
                            </div>
